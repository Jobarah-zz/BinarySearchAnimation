...
export default class AnimatedArray extends React.Component {
    ...
    animateSearch(array, number, lastChecked, start, end) {
        ...
        const b = array[half].props.children;
        /*  animation ops   */
        const updatedArray = this.state.array;
        const activeElement = <div className="element active" key={ half }>{ half }</div>;

        updatedArray[half] = activeElement;
        this.setState({array: updatedArray});

        if (number === b) {
            const foundElement = <div className="element found" key={ half }>{ half }</div>;
         ...
         <div>
          ...
          <div className="array-form">
              <div className="array-sz-section">
                 ...
                 <button type="submit" onClick={ () => this.updateSize(parseInt(this.arraySize.value, 10)) }>set Size</button>
              </div>
              <div className="array-search-section">
                 ...
                 <button type="submit" onClick={ () => { this.animateSearch(this.state.array, parseInt(this.searchValue.value, 10), 0, 0, this.state.array.length); }}>Search</button>
              </div>
          </div>
         </div>
        );
